...
const structuredAnswerPreviewLimit = 20
...
// TryBuildStructuredDataAnswer 对明确的 CSV/XLSX 查询生成本地确定性回答；未命中时交回原聊天链路。
func (s *AppService) TryBuildStructuredDataAnswer(req model.ChatCompletionRequest) (string, []map[string]string, bool, error) {
	...
	return content, structuredDataSources(tables), true, nil
}
...
func looksLikeStructuredDataQuery(query string) bool {
	...
	return containsAnyStructuredText(query, []string{
		"表格", "数据", "记录", "行", "条", "字段", "列", "名单", "展示", "列出", "筛选",
		"查找", "查询", "统计", "分布", "多少", "数量", "总数", "总共有", "员工", "人员", "教师", "老师", "用户",
		"平均", "均值",
		"最大", "最小", "最高", "最低", "最多", "最少",
	})
}

// resolveStructuredTableDocuments 严格按文档、知识库、全局唯一文档三级范围解析，避免跨表猜测。
...
func detectStructuredFilter(query string, documents []structuredTableDocument) (string, string) {
	for _, header := range allStructuredHeaders(documents) {
		...
		remainder := strings.TrimSpace(query[index+len(header):])
		for _, marker := range []string{"等于", "为", "是", "=", "：", ":"} {
			if !strings.HasPrefix(remainder, marker) {
				continue
			}
			value := trimStructuredQueryValue(strings.TrimSpace(strings.TrimPrefix(remainder, marker)))
...
func isStructuredQuestionValue(value string) bool {
	...
	for _, marker := range []string{"多少", "几", "什么", "谁", "哪", "哪个", "哪些"} {
...
func detectStructuredTargetField(query string, documents []structuredTableDocument) string {
	...
	for _, header := range headers {
		switch {
		case strings.Contains(header, "薪资") && containsAnyStructuredText(query, []string{"工资", "收入", "薪水"}):
			return header
		case strings.Contains(header, "年龄") && containsAnyStructuredText(query, []string{"年纪", "岁数"}):
			return header
		case strings.Contains(header, "教龄") && containsAnyStructuredText(query, []string{"教学年限", "任教年限", "工作年限"}):
			return header
		case strings.Contains(header, "编号") && containsAnyStructuredText(query, []string{"工号", "员工号", "教师号"}):
			return header
		case strings.Contains(header, "姓名") && containsAnyStructuredText(query, []string{"名字", "人员", "教师", "老师", "员工"}):
			return header
		}
	}
	return inferNumericStructuredTargetField(query, documents)
}

func inferNumericStructuredTargetField(query string, documents []structuredTableDocument) string {
	switch {
	case containsAnyStructuredText(query, []string{"薪资", "工资", "收入", "薪水", "薪酬"}):
		return firstStructuredHeaderContaining(documents, []string{"薪资", "工资", "收入", "薪水", "薪酬"})
	case containsAnyStructuredText(query, []string{"年龄", "年纪", "岁数"}):
		return firstStructuredHeaderContaining(documents, []string{"年龄", "年纪", "岁数"})
	case containsAnyStructuredText(query, []string{"教龄", "教学年限", "任教年限"}):
		return firstStructuredHeaderContaining(documents, []string{"教龄", "教学年限", "任教年限"})
	case containsAnyStructuredText(query, []string{"分数", "成绩", "得分"}):
		return firstStructuredHeaderContaining(documents, []string{"分数", "成绩", "得分"})
	case containsAnyStructuredText(query, []string{"价格", "金额", "费用", "成本"}):
		return firstStructuredHeaderContaining(documents, []string{"价格", "金额", "费用", "成本"})
	default:
		return ""
	}
}

func firstStructuredHeaderContaining(documents []structuredTableDocument, markers []string) string {
	for _, header := range allStructuredHeaders(documents) {
...
func trimStructuredQueryValue(value string) string {
	value = strings.Trim(strings.TrimSpace(value), " ，。；;、?？!！'\"`")
	for _, stopper := range []string{"的", "，", "。", "；", ";", "、", "?", "？", "!", "！", "并且", "然后"} {
...
func isStructuredCountQuestion(query string) bool {
	return containsAnyStructuredText(query, []string{"多少", "几", "数量", "总数", "共", "总共有"}) &&
		containsAnyStructuredText(query, []string{"记录", "行", "条", "数据", "人员", "名单", "用户", "教师", "老师", "员工"})
}
...
func renderStructuredPreviewAnswer(query string, documents []structuredTableDocument) string {
	...
	if containsAnyStructuredText(query, []string{"完整", "全部", "所有"}) {
		limit *= 2
	}
...
func renderStructuredExtremumAnswer(plan structuredQueryPlan, documents []structuredTableDocument) string {
	...
	for _, match := range collectStructuredRows(documents, "", "") {
		index := structuredHeaderIndex(match.Table.Headers, targetField)
...
%s`, title, targetField, formatStructuredNumber(bestValue), renderStructuredMarkdownTable(best.Table.Headers, []util.StructuredTableRow{best.Row})))
}
...
func renderStructuredGroupAnswer(plan structuredQueryPlan, documents []structuredTableDocument) string {
	...
	items := make([]groupItem, 0, len(counts))
	for value, count := range counts {
		items = append(items, groupItem{Value: value, Count: count})
	}
	...
	for _, item := range items {
		fmt.Fprintf(builder, "|%s|%d|\n", escapeStructuredMarkdownCell(item.Value), item.Count)
	}
...
func collectStructuredRows(documents []structuredTableDocument, field, value string) []structuredRowMatch {
	...
	for _, document := range documents {
		for _, table := range document.Tables {
			filterIndex := structuredHeaderIndex(table.Headers, field)
			for _, row := range table.Rows {
				if filterIndex >= 0 {
					if filterIndex >= len(row.Values) || !strings.EqualFold(strings.TrimSpace(row.Values[filterIndex]), strings.TrimSpace(value)) {
						continue
					}
				}
...
func renderStructuredRowsAnswer(title string, matches []structuredRowMatch, limit int) string {
	...
	for _, match := range matches[:limit] {
		if !sameStructuredTable(currentTable, match.Table) {
			builder.WriteString(renderStructuredMarkdownTable(currentTable.Headers, rows))
			builder.WriteString("\n\n")
...
func renderStructuredMarkdownTable(headers []string, rows []util.StructuredTableRow) string {
	builder := &strings.Builder{}
	builder.WriteString("|")
	for _, header := range headers {
		builder.WriteString(escapeStructuredMarkdownCell(header))
		builder.WriteString("|")
	}
	builder.WriteString("\n|")
	for range headers {
	...
	for _, row := range rows {
		...
		for index := range headers {
			...
			builder.WriteString(escapeStructuredMarkdownCell(value))
			builder.WriteString("|")
		}
		builder.WriteString("\n")
	}
...
func structuredHeaderIndex(headers []string, field string) int {
...
func escapeStructuredMarkdownCell(value string) string {
...
func containsAnyStructuredText(text string, markers []string) bool {
	for _, marker := range markers {
