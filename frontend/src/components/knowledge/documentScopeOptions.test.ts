...
describe('getDocumentScopeMatches', () => {
  ...
  it('keeps the selected document visible outside the first result window', () => {
  ...
  it('does not keep a selected document that misses the active query', () => {
    const documents = [
  ...
  it('supports an explicit zero result limit', () => {
    const result = getDocumentScopeMatches(createDocuments(3), '', 'doc-2', 0)
    expect(result).toEqual({ total: 3, visible: [] })
  })
})
